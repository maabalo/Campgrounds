...
if (btnUploadMode) {
  btnUploadMode.addEventListener('click', () => {
    ...
    // Clear preview if URL mode had invalid data, otherwise keep existing file data
...
if (btnUrlMode) {
  btnUrlMode.addEventListener('click', () => {
    ...
    // Clear preview if file mode was active, unless URL input already has a value
    if (!editImgUrl.value) {
      resetImagePreview();
    }
  });
...
function openEditorModal(camp = null) {
  ...
  if (camp) {
    ...
    // Pull existing image data into memory
    ...
    if (currentImageData && currentImageData.trim() !== '') {
      ...
      // Route based on image data type
    ...
    // Reset helper text back to default
    const helperText = fileUploadSection ? fileUploadSection.querySelector('.helper-text') : null;
    if (helperText) {
      helperText.textContent = 'Picks photo from device (Auto-compressed < 500KB)';
    }
